#!/usr/bin/env bash
#
# Clean, rebuild, and run FleaTrackr.App.
...
step "Running FleaTrackr.App  (Ctrl+C to stop)"
# --no-build: run exactly what was just built, with no implicit rebuild.
dotnet run --project Src/FleaTrackr.App --configuration "$CONFIGURATION" --no-build
